...
from my_utils import word_ordering_prob
...
class ConstrainedHypothesis:
    ...
    def __init__(self,
                 ...
                 dep_constraint_list: List[List[str]],
                 eos_id: int,
                 **model_kwargs) -> None:
        ...
        self.force_complete = model_kwargs['force_complete']
        self.reset_avail_states = model_kwargs['reset_avail_states']

        self.constraints = []  # type: List[int]
        if model_kwargs.get('lower') == False:
            self.dep_constraints_dict = {(d[0], d[1], d[2]): False
            for d in dep_constraint_list} # type: List[List[str]]
        else:
            self.dep_constraints_dict = {(d[0].lower(), d[1], d[2].lower()): False
                for d in dep_constraint_list} # type: List[List[str]]
        self.col_to_info = {}
        self.dep_correct_met = set()
        self.total_weight = 0.0

        self.have_overlap = False
        self.raw_lexical_constraints = set()
        for w1, r, w2 in dep_constraint_list:
            if w1 in self.raw_lexical_constraints or w2 in self.raw_lexical_constraints:
                self.have_overlap = True
            self.raw_lexical_constraints.add(w1)
            self.raw_lexical_constraints.add(w2)

        # debug
        self.have_overlap = False

        # print(self.have_overlap)
        # print(dep_constraint_list)

        # related_dep_constraints:
        self.related_dep_constraints = {}
        for w1, r, w2 in self.dep_constraints_dict:
            if w1 not in self.related_dep_constraints:
                self.related_dep_constraints[w1] = []
            if w2 not in self.related_dep_constraints:
                self.related_dep_constraints[w2] = []
            self.related_dep_constraints[w1].append((w1, r, w2))
            self.related_dep_constraints[w2].append((w1, r, w2))

        # # build_word_ordering_graph
        # self.word_ordering_graph = {}
        # for w1, r, w2 in self.dep_constraints_dict:
        #     if word_ordering_prob[r] > 0.5:
        #         pre_word, post_word = w1, w2
        #     else:
        #         pre_word, post_word = w2, w1
        #     if pre_word not in self.word_ordering_graph:
        #         self.word_ordering_graph[pre_word] = []
        #     self.word_ordering_graph[pre_word].append(post_word)

        # # find all words whose in-degree=0
        # now_not_available = set()
        # for pre_word in self.word_ordering_graph:
        #     for post_word in self.word_ordering_graph[pre_word]:
        #         # except when pre word == post word
        #         if pre_word == post_word:
        #             continue
        #         now_not_available.add(post_word)
        available_words = []
        # for pre_word in self.word_ordering_graph:
        #     if pre_word not in now_not_available:
        #         available_words.append(pre_word)
        
        # token_ids = tokenizer([' '+w for w in self.raw_lexical_constraints], add_special_tokens=False)["input_ids"]
        # self.token_id_to_word = {tuple(token_id): word.lower() 
        #     for token_id, word in zip(token_ids, list(self.raw_lexical_constraints))}
        # self.word_to_token_id = {v: k for k, v in self.token_id_to_word.items()}

        # # may have multiple spans 
        self.token_id_to_spans = {}
        ...
        for phrase in constraint_list:
            ...
            cur_span = [len(self.constraints)-len(phrase), len(self.constraints)]
            if tuple(phrase) not in self.token_id_to_spans:
                self.token_id_to_spans[tuple(phrase)] = []
            self.token_id_to_spans[tuple(phrase)].append(cur_span)
            if self.have_overlap:
                if self.token_id_to_word[tuple(phrase)] in available_words:
                    self.is_available += [True] * len(phrase)
                else:
                    self.is_available += [False] * len(phrase)
            else:
                self.is_available += [True] * len(phrase)

        # self.raw_lexical_constraints = []
        # for c in self.constraints:
        #     raw_word = tokenizer.decode(c, skip_special_tokens=True, 
        #         clean_up_tokenization_spaces=False)
        #     if len(raw_word) >= 2 and raw_word[0] == ' ':
        #         raw_word = raw_word[1:]
        #     raw_word = raw_word.lower()
        #     self.raw_lexical_constraints.append(raw_word)

        # print(self.raw_lexical_constraints)
        # print(self.constraints)
        # print(self.token_id_to_word)
        # print(self.word_to_token_id)
        # print(self.is_sequence)
        # print(self.is_available)
        # print(self.token_id_to_spans)
        # print(self.word_ordering_graph)
        # pdb.set_trace()
    ...
    def dep_num_met(self) -> int:
        return sum(state for k, state in self.dep_constraints_dict.items())

    def dep_num_correct_met(self) -> int:
        return len(self.dep_correct_met)

    def num_needed(self) -> int:
        """
        :return: the number of un-met lexical constraints.
        """
    ...
    def is_valid(self, wordid, k=None) -> bool:
        ...
        # return wordid != self.eos_id or (self.num_needed() == 1 and self.eos_id in self.allowed())
    ...
    def simple_lemmatize(self, wordid) -> str:
        if not self.have_overlap:
            return wordid
        raw_word = self.tokenizer.decode(wordid, skip_special_tokens=True, 
            clean_up_tokenization_spaces=False)
        if len(raw_word) >= 2 and raw_word[0] == ' ':
            raw_word = raw_word[1:]
        raw_word = raw_word.lower()
    ...
    def is_available_word(self, wordid) -> bool:
        if not self.have_overlap:
            return True
        raw_word = self.simple_lemmatize(wordid)
        if raw_word in self.raw_lexical_constraints:
            if not sum(avail for avail, w in zip(self.is_available, self.raw_lexical_constraints) if raw_word == w):
                # print(wordid, self.is_available, self.constraints)
                # print('False:', raw_word, self.is_available, self.raw_lexical_constraints)
                return False
        # if wordid == 653:
        #     print('True:', raw_word, self.is_available, self.raw_lexical_constraints)
        return True

    def update_available_state(self, pos) -> None:
        # check if all the word in span have met
        # print('met:', self.raw_lexical_constraints[pos])
        if not self.have_overlap:
            return
        for token_id, spans in self.token_id_to_spans.items():
            for span in spans:
                if span[0] <= pos < span[1]:
                    all_met = sum(self.met[span[0]: span[1]]) == span[1] - span[0]
                    if all_met:
                        # set available state
                        if self.reset_avail_states:
                            for i in range(span[0], span[1]):
                                self.is_available[i] = False
                        word = self.token_id_to_word[token_id]
                        for post_word in self.word_ordering_graph.get(word, []):
                            post_word_ids = self.word_to_token_id[post_word]
                            post_word_spans = self.token_id_to_spans[post_word_ids]
                            for post_word_span in post_word_spans:
                                if post_word_span == span:
                                    continue
                                for i in range(post_word_span[0], post_word_span[1]):
    ...
    def advance(self, word_id: int) -> 'ConstrainedHypothesis':
        ...
        if obj.last_met != -1 and obj.is_sequence[obj.last_met] == 1:
            if word_id == obj.constraints[obj.last_met + 1]:
                ...
                obj.update_available_state(obj.last_met + 1)
                obj.last_met += 1
            else:
        ...
        # If not, check whether we're meeting a single-word constraint
            ...
            if self.have_overlap:
                constraint_tuples = list(zip(obj.raw_lexical_constraints, [False] + obj.is_sequence[:-1], obj.met))
            else:
            ...
            query = (self.simple_lemmatize(word_id), False, False)
            try:
                pos = constraint_tuples.index(query)
                obj.met[pos] = True
                obj.last_met = pos
                obj.update_available_state(pos)
            except ValueError:
...
# TODO
def init_dep_constraints(raw_constraints: List[Optional[RawConstraintList]],
                     dep_constraints: List[Optional[RawConstraintList]],
                     beam_size: int,
                     start_id: int,
                     eos_id: int,
                     **model_kwargs) -> List[Optional[ConstrainedHypothesis]]:
    """
    ...
    if any(raw_constraints):
        for i, raw_list in enumerate(raw_constraints):
            ...
            if num_constraints > 0:
                hyp = ConstrainedHypothesis(raw_list, dep_constraints[i], eos_id, **model_kwargs)
                idx = i * beam_size
...
def dep_dba_topk(input_ids, batch_size, beam_size, inactive, scores, hypotheses, best_ids, best_word_ids, seq_scores, debug=False,
    ...
    for sentno in range(batch_size):
        ...
        if hypotheses[rows.start] is not None and hypotheses[rows.start].size() > 0:
            ...
            hypotheses[rows], inactive[rows] = _sequential_topk(input_ids[rows],
                                                                beam_size,
                                                                inactive[rows],
                                                                scores[rows],
                                                                hypotheses[rows],
...
def _sequential_topk(input_ids,
                     beam_size,
                     inactive,
                     scores,
                     hypotheses,
                     ...
                     attn_scores=None, score_mode=0, **model_kwargs):
    if model_kwargs['bank_count'] == 'dep':
        num_constraints = len(hypotheses[0].dep_constraints_dict.keys())
    elif model_kwargs['bank_count'] == 'word+dep':
        num_constraints = hypotheses[0].size() + len(hypotheses[0].dep_constraints_dict.keys())
    else:
    ...
    for row, col, seq_score in zip(best_ids, best_word_ids, sequence_scores):
        ...
        if partial:
            ...
            valid = hypotheses[row].is_valid_full(col)# and hypotheses[row].is_available_word(col)
        if valid :
    ...
    for row in range(beam_size):
        ...
        for col in nextones:
            # if col == 653:
            #     pdb.set_trace()
            # if not hyp.is_available_word(col):
            #     continue
    ...
    sorted_candidates = scorer_factory[model_kwargs['scorer']](
        parser=model_kwargs['parser'],
        sorted_candidates=list(sorted_candidates), 
        input_ids=input_ids,
        beam_size=beam_size, 
        prune_size=model_kwargs['prune_size'],
    ...
    for cand in sorted_candidates:
        if model_kwargs['bank_count'] == 'dep':
            bank = cand.hypothesis.dep_num_correct_met()
        elif model_kwargs['bank_count'] == 'word+dep':
            bank = cand.hypothesis.num_met() + cand.hypothesis.dep_num_correct_met()
        else:
