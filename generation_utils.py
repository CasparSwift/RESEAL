...
from RESEAL import init_dep_constraints, dep_dba_topk
from dep_scorer import prob_adjust, webnlg_prob_adjust
...
def generate(
    ...
    # (PVP): this should later be handled by the forward fn() in each model in the future see PR 3140
    ...
    if self.config.is_encoder_decoder:
        ...
        if 'use_copy' not in model_kwargs or not model_kwargs['use_copy']:
            output_a_h = False
        else:
            output_a_h = True
        encoder_outputs = encoder(input_ids, attention_mask=attention_mask, return_dict=True,
            output_attentions=output_a_h, output_hidden_states=output_a_h)
    ...
    if self.config.is_encoder_decoder:
        real_input_ids = input_ids  # NB. store the encoder_side input_ids for copy
    ...
    if num_beams > 1:
        output = _generate_beam_search(self,
            input_ids,
            real_input_ids,
        ...
        output = _generate_no_beam_search(self,
            input_ids,
            real_input_ids,
            cur_len=cur_len,
            max_length=max_length,
...
def _generate_no_beam_search(
    ...
    while cur_len < max_length:
        ...
        if 'use_copy' in model_kwargs and model_kwargs['use_copy']:
            model_inputs['input_ids'] = real_input_ids
        ...
        # for ct, i in enumerate(input_ids):
        #     # token_id = tokens_to_add[ct]
        #     # print(next_token_logits[ct, token_id])
        #     print(ct, tokenizer.decode(i))
        # print('*' * 30)
...
def _generate_beam_search(
    ...
    inactive = torch.zeros(batch_size * num_beams).to(input_ids.device)
    ...
    if have_lexical_constraints and have_dep_constraints:
        constraints = init_dep_constraints(lexical_constraints, dep_constraints, num_beams, 0, eos_token_id, **model_kwargs)
    elif have_lexical_constraints and not have_dep_constraints:
        constraints = init_constraints(lexical_constraints, num_beams, 0, eos_token_id)
    else:
        constraints = None
    ...
    while cur_len < max_length:
        ...
        if self.config.is_encoder_decoder and do_sample is False:
            # (PVP) still a bit hacky here - there might be a better solution
        ...
        # NB. changed for constrained decoding
        if do_sample:
            _scores = scores + beam_scores[:, None].expand_as(scores)  # (batch_size * num_beams, vocab_size)
            # Temperature
            if temperature != 1.0:
                _scores = _scores / temperature
            curr_scores = _scores.clone()
            ...
            next_tokens = torch.multinomial(probs, num_samples=num_beams)  # (batch_size, num_beams * 2)
            # Compute next scores
            ...
            if model_kwargs['scorer'] == 'prob':
                scores = prob_adjust(scores, beam_scores, constraints, input_ids, num_beams, **model_kwargs)
            elif model_kwargs['scorer'] == 'webnlg_prob':
                scores = webnlg_prob_adjust(scores, beam_scores, constraints, input_ids, num_beams, **model_kwargs)
        ...
        # DEBUG = False
        # if DEBUG:
        #     for ct, sample in enumerate(input_ids):
        #         cur_beam_score = beam_scores[ct].item()
        #         print(ct % num_beams, f'{cur_beam_score:.2f}\t', tokenizer.decode(sample))
        #     print()
        #     for ct, cur_hyo in enumerate(generated_hyps[0].beams):
        #         print(ct % num_beams, f'{cur_hyo[0]:.2f}\t', tokenizer.decode(cur_hyo[1]))
        #     print('-' * 30)
    ...
    # if DEBUG:
    #     dec = tokenizer.batch_decode(decoded, skip_special_tokens=True, clean_up_tokenization_spaces=False)
    #     for i in dec:
    #         print('best:', i)
    return decoded
